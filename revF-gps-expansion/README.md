...
Open `owasso1.kicad_pro` in this directory. The project one directory above is
...
[connection review](../review-2026-09-20/CONNECTION_REVIEW.md).
...
To repeat the read-only contract checks, run `python verify_revF.py` from the
parent directory after exporting a fresh `netlist.xml`. The build/routing and
layout experiment scripts in the parent directory are checkpoint tools, not an
